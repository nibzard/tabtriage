...
/**
 * Generate embeddings using Jina Embeddings v3 via Hugging Face Inference API
 * This avoids local model dependencies and Sharp issues
 */
async function generateEmbedding(
  text: string, 
  task: string = 'text-matching',
  truncateDim: number = 1024
): Promise<number[]> {
  try {
    // Use Hugging Face Inference API for Jina v3
    const response = await fetch(
      'https://api-inference.huggingface.co/models/jinaai/jina-embeddings-v3',
      {
        method: 'POST',
        headers: {
    ...
    // The API returns a single embedding array
    if (Array.isArray(embedding) && embedding.length > 0) {
      let embeddingArray = embedding[0];
      
      // Ensure we have the right dimension
      if (embeddingArray.length > truncateDim) {
        embeddingArray = embeddingArray.slice(0, truncateDim);
        
        // Re-normalize after truncation
        const norm = Math.sqrt(embeddingArray.reduce((sum: number, val: number) => sum + val * val, 0));
        embeddingArray = embeddingArray.map((val: number) => val / norm);
      }
      
      logger.debug(`Generated ${embeddingArray.length}-dimensional Jina v3 embedding via API (task: ${task})`);
      return embeddingArray;
    }
    
    throw new Error('Invalid embedding response from API');
  } catch (error) {
    logger.error('Error generating Jina v3 embedding via API:', error);
    
    // Fallback to normalized random vector
    logger.warn('Falling back to random embedding vector');
    const fallbackVector = Array(truncateDim).fill(0).map(() => Math.random() * 2 - 1);
    const norm = Math.sqrt(fallbackVector.reduce((sum, val) => sum + val * val, 0));
    return fallbackVector.map(val => val / norm);
  }
}
...
/**
 * Update tab with Jina v3 embedding via API
 */
export async function updateTabEmbedding(
  ...
  try {
    // Generate embedding from text using Jina v3 API
    ...
    logger.info(`Updated Jina v3 embedding for tab ${tabId} via API (task: ${task})`);
  } catch (error) {
...
/**
 * Search tabs using Jina v3 vector similarity via API
 */
export async function searchTabsByVector(
  ...
  try {
    ...
    logger.error('Error in Jina v3 vector search via API:', error);
...
export async function hybridSearch(
  ...
  try {
    // Get vector search results using Jina v3 API
    ...
    logger.error('Error in Jina v3 hybrid search via API:', error);
...
/**
 * Batch update embeddings for tabs without embeddings using Jina v3 API
 */
export async function updateMissingEmbeddings(
  ...
  try {
    ...
    logger.info(`Updated Jina v3 embeddings for ${updatedCount} tabs via API`);
    return updatedCount;
  } catch (error) {
    logger.error('Error updating missing Jina v3 embeddings via API:', error);
    throw error;
  }
}
